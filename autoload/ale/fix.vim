...
function! s:HandleExit(job_info, buffer, output, data) abort
    ...
    " Remove the job ID.
    call filter(l:buffer_info.job_list, 'v:val isnot# a:data.job_id')

    if a:job_info.read_temporary_file
        let a:job_info.output = !empty(a:data.temporary_file)
        \   ?  readfile(a:data.temporary_file)
        \   : []
    else
        let a:job_info.output = a:output
    endif
    ...
    if l:ProcessWith isnot v:null
        let a:job_info.output = call(
        \   ale#util#GetFunction(l:ProcessWith),
        \   [a:buffer, a:job_info.output]
        \)
    endif
    ...
    if l:ChainCallback is v:null && !empty(split(join(a:job_info.output)))
        let l:input = a:job_info.output
    else
...
function! s:RunJob(options) abort
    ...
    \   'output_stream': a:options.read_temporary_file
    \       ? a:options.output_stream
    \       : 'none',
    \   'executable': '',
    ...
    if l:job_id
        call add(g:ale_fix_buffer_data[l:buffer].job_list, l:job_id)
    endif
...
function! ale#fix#Fix(buffer, fixing_flag, ...) abort
    ...
    if has_key(g:ale_fix_buffer_data, a:buffer)
        ...
        let g:ale_buffer_info[a:buffer].job_list = []
    endif
